...
class ParsedDocumentDetailView(RetrieveAPIView):
    ...
    def get_object(self):
        return DocumentParsingService.get_parsed_for_document(self.kwargs["document_id"])
...
class ParsedDocumentPagesView(ListAPIView):
    ...
    def get_queryset(self):
        parsed = DocumentParsingService.get_parsed_for_document(self.kwargs["document_id"])
...
class ParsedDocumentPageDetailView(APIView):
    def get(self, request, document_id, page_number):
        parsed = DocumentParsingService.get_parsed_for_document(document_id)
        try:
