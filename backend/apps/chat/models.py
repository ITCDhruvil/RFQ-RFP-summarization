...
class DocumentVectorIndex(UUIDPrimaryKeyModel, TimeStampedModel):
    """Tracks Chroma indexing state for a document's chunks."""
    ...
    # Stored chunk IDs so we can delete stale vectors by ID instead of
    # doing a slow metadata-filter scan across the whole collection.
    indexed_chunk_ids = models.JSONField(
        default=list,
        blank=True,
        help_text="Chroma chunk IDs currently indexed for this document.",
    )
