...
class TenderService:
    ...
    @staticmethod
    def get_tender(tender_id: UUID) -> Tender:
        try:
    ...
    @staticmethod
    def list_tenders():
        return Tender.objects.prefetch_related("document_versions__document")

    @staticmethod
    @transaction.atomic
    def resolve_tender(ctx: TenderUploadContext) -> Tender:
        if ctx.tender_id:
            return TenderService.get_tender(ctx.tender_id)
